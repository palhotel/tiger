...
public class TranslateVisitor implements codegen.C.Visitor {
	...
	public void visit(codegen.C.exp.ArraySelect e) {
		...
		e.index.accept(this);
		emit(new cfg.stm.ArraySelect(dst, null, array, this.operand));
		this.operand = new cfg.operand.Var(dst);
		return;
	}
	...
	public void visit(codegen.C.exp.NewIntArray e) {
		String dst = genVar(new cfg.type.Int());
		e.exp.accept(this);
		emit(new cfg.stm.NewArray(dst, this.operand));
		this.operand = new cfg.operand.Var(dst);
		return;
	}
	...
	public void visit(codegen.C.exp.Not e) {
		e.exp.accept(this);
		emit(new cfg.stm.Not(genVar(), null, this.operand));
		return;
	}
	...
	public void visit(codegen.C.stm.AssignArray s) {
		...
		s.exp.accept(this);
		emit(new cfg.stm.MoveArray(s.id, index, null, this.operand));
		return;
	}
	...
	public void visit(codegen.C.stm.While s) {
		util.Label sl = new util.Label(), tl = new util.Label(), fl = new util.Label();
		emit(new cfg.transfer.Goto(sl));
		emit(sl);
		s.condition.accept(this);
		emit(new cfg.transfer.If(this.operand, tl, fl));
		emit(tl);
